...
async function generatePDFReport(report, data) {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.fontSize(10).text(`${formatDate(data.summary.dateRange.startDate)} - ${formatDate(data.summary.dateRange.endDate)}`, { align: 'center' });
            doc.moveDown(1);
            // Summary Section
            ...
            for (let i = 0; i <= currentPageIndex; i++) {
                if (i > 0)
                    doc.switchToPage(i);
                doc.fontSize(8).text(`${footerText} - Page ${i + 1} of ${totalPages}`, 50, doc.page.height - 60, { align: 'center', width: doc.page.width - 100 });
            }
            doc.end();
        }
        catch (error) {
            logger_1.default.error('PDF generation failed:', error);
            reject(error);
        }
    });
}
async function generateCSVReport(data) {
    try {
        ...
        logger_1.default.error('CSV generation failed:', error);
        throw error;
    }
}
async function generateExcelReport(report, data) {
    try {
        ...
        logger_1.default.error('Excel generation failed:', error);
        throw error;
    }
}
// Utility functions
