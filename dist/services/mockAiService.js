...
class MockAIService {
    ...
    async initializeFallbackRules() {
        try {
            ...
            for (const rule of ruleDefinitions) {
                ...
                if (!category) {
                    logger_1.default.info(`Creating missing category: ${rule.categoryName}`);
                    try {
                        ...
                        logger_1.default.info(`Successfully created category: ${rule.categoryName} with ID: ${category.id}`);
                    }
                    catch (error) {
                        logger_1.default.error(`Failed to create category ${rule.categoryName}:`, error);
                        continue;
                    }
                }
                // Store the rule
                ...
                logger_1.default.debug(`Initialized rule for category: ${rule.categoryName} (${category.id}) with ${rule.patterns.length} patterns`);
            }
            logger_1.default.info(`Initialized ${this.fallbackRules.size} mock AI categorization rules`);
            // Log all initialized categories for debugging
            logger_1.default.debug('All initialized categories:');
            for (const [categoryId, rule] of this.fallbackRules) {
                const category = categories.find(c => c.id === categoryId);
                logger_1.default.debug(`- ${category?.name || 'Unknown'} (${categoryId}): ${rule.patterns.length} patterns`);
            }
        }
        catch (error) {
            logger_1.default.error('Failed to initialize mock AI rules:', error);
        }
    }
    getDefaultColorForCategory(categoryName) {
        const colorMap = {
    ...
    async categorizeExpense(data) {
        ...
        logger_1.default.info(`Categorizing expense with search text: "${searchText}"`);
        ...
        for (const [categoryId, rule] of this.fallbackRules) {
            for (let i = 0; i < rule.patterns.length; i++) {
                ...
                logger_1.default.debug(`Testing pattern ${i + 1} for category ${categoryId}: ${pattern.source} -> ${match ? 'MATCH' : 'NO MATCH'}`);
                if (match) {
                    ...
                    logger_1.default.info(`Mock AI categorization successful: ${categoryName} (${categoryId}) with confidence ${finalConfidence.toFixed(2)}`);
                    logger_1.default.info(`Matched pattern: ${pattern.source}`);
                    return {
                        categoryId,
        ...
        logger_1.default.warn(`No patterns matched for search text: "${searchText}"`);
        logger_1.default.info('Available categories and their first patterns:');
        for (const [categoryId, rule] of this.fallbackRules) {
            const category = await prisma.category.findUnique({ where: { id: categoryId } });
            logger_1.default.info(`- ${category?.name || 'Unknown'}: ${rule.patterns[0]?.source || 'No patterns'}`);
        }
        // Fallback to default category
        return await this.getDefaultCategory();
    }
    generateReasoning(data, matchedPattern, categoryName) {
    ...
    async learnFromCorrection(originalCategoryId, correctedCategoryId, expenseData) {
        ...
        logger_1.default.info(`Mock AI learned from correction: ${originalCategoryId} -> ${correctedCategoryId}`);
        ...
        try {
            ...
            logger_1.default.error('Failed to store mock learning data:', error);
        }
    }
    ...
    async testConnection() {
        ...
        logger_1.default.info('Mock AI service connection test: successful');
        return true;
    }
    ...
    async testPatternMatching(searchText) {
        const results = [];
...
exports.mockAiService = new MockAIService();
